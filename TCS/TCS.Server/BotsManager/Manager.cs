...
public class Manager
{
    ...
    public static async Task ConnectBot(int id, string botUsername, DatabaseContext db)
    {
        if (!users.TryGetValue(id, out var user))
        {
            var streamerUsername =
                await db.Configurations.Where(x => x.Id == id).Select(x => x.StreamerUsername).FirstAsync();
    ...
    public static async Task DisconnectBot(int id, string botUsername, DatabaseContext db)
    {
        if (!users.TryGetValue(id, out var user))
        {
            var streamerUsername =
    ...
    public static async Task ConnectAllBots(int id, DatabaseContext db)
    {
        if (!users.TryGetValue(id, out var user))
        {
            var streamerUsername =
    ...
    public static async Task DisconnectAllBots(int id, DatabaseContext db)
    {
        if (!users.TryGetValue(id, out var user))
        {
            var streamerUsername =
    ...
    public static async Task<bool> Send(int id, string botUsername, string message, string replyTo, DatabaseContext db)
    {
        if (!users.TryGetValue(id, out var user))
        {
            var streamerUsername =
    ...
    public static async Task StartSpam(int id, int threads, int delay, string[] messages, SpamMode mode,
        DatabaseContext db)
    {
        if (!users.TryGetValue(id, out var user))
        {
