...
public class Bot(string username, string token, string streamerUsername, Proxy proxy)
{
    ...
    public async Task Connect()
    {
        ...
        try
        {
            await connection.ConnectAsync(new Uri("wss://irc-ws.chat.twitch.tv:443"),
                new CancellationTokenSource(5000).Token);
            await connection.SendAsync(
                new ArraySegment<byte>(Encoding.UTF8.GetBytes("CAP REQ :twitch.tv/tags twitch.tv/commands")),
                WebSocketMessageType.Text, true, new CancellationTokenSource(5000).Token);
            await connection.SendAsync(new ArraySegment<byte>(Encoding.UTF8.GetBytes("PASS oauth:" + token)),
                WebSocketMessageType.Text, true, new CancellationTokenSource(5000).Token);
            await connection.SendAsync(new ArraySegment<byte>(Encoding.UTF8.GetBytes($"NICK {username}")),
                WebSocketMessageType.Text, true, new CancellationTokenSource(5000).Token);
            await connection.SendAsync(
                new ArraySegment<byte>(Encoding.UTF8.GetBytes($"USER {username} 8 *:{username}")),
                WebSocketMessageType.Text, true, new CancellationTokenSource(5000).Token);
            await connection.SendAsync(new ArraySegment<byte>(Encoding.UTF8.GetBytes($"JOIN #{streamerUsername}")),
                WebSocketMessageType.Text, true, new CancellationTokenSource(5000).Token);
    ...
    {
        if (connection.State != WebSocketState.Open) throw new Exception("Connection is not open");
        await connection.SendAsync(new ArraySegment<byte>(Encoding.UTF8.GetBytes("PING")), WebSocketMessageType.Text,
            true, cancellationToken);
        var buffer = new byte[1024];
        while (!cancellationToken.IsCancellationRequested)
        {
            var received = await connection.ReceiveAsync(new ArraySegment<byte>(buffer), cancellationToken);
            if (received.MessageType == WebSocketMessageType.Close) throw new Exception("Connection is not open");
            var message = Encoding.UTF8.GetString(buffer, 0, received.Count);
            if (message.Contains("PING"))
            {
                await connection.SendAsync(new ArraySegment<byte>(Encoding.UTF8.GetBytes("PONG")),
        ...
        try
        {
            ...
            await connection.SendAsync(
                new ArraySegment<byte>(Encoding.UTF8.GetBytes(
                    $"{(replyTo is not null ? $"@reply-parent-msg-id={replyTo} " : string.Empty)}PRIVMSG #{streamerUsername} :{message}")),
                WebSocketMessageType.Text, true, ctoken.Token);
        }
        catch (Exception ex)
        {
            await Connect();
