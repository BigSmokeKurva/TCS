...
public class BrowserThread
{
    ...
    private async Task Bot()
    {
        ...
        try
        {
            ...
            var proxy = Item.Proxy.Type == "socks5"
                ? new Proxy { Server = $"http://127.0.0.1:{proxyServer.ProxyEndPoints[0].Port}" }
                : new Proxy
                {
                    Server = $"http://{Item.Proxy.Host}:{Item.Proxy.Port}",
                    Username = Item.Proxy.Credentials.Value.Username, Password = Item.Proxy.Credentials.Value.Password
                };
            // Запуск браузера
            ...
            page.RequestFinished += async (sender, e) =>
            {
                if (e.Url == "https://gql.twitch.tv/integrity" && e.Method == "POST" && !returned)
                    try
                    {
                        ...
                        {
                            Proxy = new WebProxy($"{Item.Proxy.Type}://{Item.Proxy.Host}:{Item.Proxy.Port}", false,
                                new string[] { }, Item.Proxy.Credentials.Value)
                        });
                        ...
                        var message = new HttpRequestMessage
                        {
                            ...
                            {
                                Actions.Unfollow => new StringContent(
                                    "[{\"operationName\": \"FollowButton_UnfollowUser\", \"variables\": {\"input\": {\"targetID\": \"" +
                                    Item.TargetId +
                                    "\"}}, \"extensions\": {\"persistedQuery\": {\"version\": 1, \"sha256Hash\": \"f7dae976ebf41c755ae2d758546bfd176b4eeb856656098bb40e0a672ca0d880\"}}}]"),
                                Actions.Follow => new StringContent(
            ...
            if (Item.State == ThreadState.Followed)
                await AddFollow();
            else if (Item.State == ThreadState.Unfollowed) await RemoveFollow();
            await Lock();
            FollowBot.Queue.Remove(Item);
            Unlock();
        }
    ...
    private async Task<ThreadState> CheckFollow()
    {
        ...
        var personalSections =
            (await (await httpClient.SendAsync(message)).Content.ReadFromJsonAsync<PersonalSectionsJson[]>())[0];
        if (personalSections.data is null) return ThreadState.Error;
        if (personalSections.data.personalSections[0].items is not null && personalSections.data.personalSections[0]
                .items.Any(x =>
                {
                    if (x.user is null) return false;
