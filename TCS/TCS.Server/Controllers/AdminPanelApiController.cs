...
public class AdminPanelApiController(DatabaseContext db) : ControllerBase
{
    ...
    private async Task<ActionResult> ChangeTokens(int id, JsonElement tokens)
    {
        ...
        var __tokens = tokens.EnumerateArray().Select(x => x.GetString().Split(':')).Distinct()
            .Where(x => x.Length == 6 || x.Length == 7) /*.ToDictionary(x => x[0], x => x[1..])*/;
    ...
    public async Task<ActionResult> GetTokens(int id, bool usernames)
    {
        ...
        if (usernames)
        {
            ...
            tokens = _tokens.Select(x =>
                $"{x.Username.PadRight(maxUsernameLength)}:{x.Token}:{x.Proxy.Type}:{x.Proxy.Host}:{x.Proxy.Port}:{x.Proxy.Credentials.Value.Username}:{x.Proxy.Credentials.Value.Password}");
        }
        else
        {
            tokens = _tokens.Select(x =>
                $"{x.Token}:{x.Proxy.Type}:{x.Proxy.Host}:{x.Proxy.Port}:{x.Proxy.Credentials.Value.Username}:{x.Proxy.Credentials.Value.Password}");
        }
